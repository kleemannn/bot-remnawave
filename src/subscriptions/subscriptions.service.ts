...
export class SubscriptionsService {
  ...
  async exportDealerUsers(dealerTelegramId: bigint): Promise<Array<{ username: string; subscriptionUrl: string }>> {
    const dealer = await this.getDealerOrThrow(dealerTelegramId);
    await this.syncSubscriptionsForDealer(dealer.id);
    ...
    const exported: Array<{ username: string; subscriptionUrl: string }> = [];
  ...
  async setSubscriptionExpiresAt(
    dealerTelegramId: bigint,
    subscriptionId: string,
    expiresAt: Date,
  ) {
